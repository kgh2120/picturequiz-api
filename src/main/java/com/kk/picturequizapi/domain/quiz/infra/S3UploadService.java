...
import com.kk.picturequizapi.domain.quiz.command.domain.Picture;
import com.kk.picturequizapi.domain.quiz.command.domain.PictureUploadService;
import com.kk.picturequizapi.domain.quiz.exception.FileTypeNotImageFileException;
...
@Service
public class S3UploadService implements PictureUploadService {
    ...
    public Picture uploadPicture(MultipartFile multipartFile) {
        try{
            ...
            String url = uploadFiles(multipartFile, "picture");
    ...
    private void validateIsClientSendImageFile(MultipartFile multipartFile) {
        if(!Objects.requireNonNull(multipartFile.getContentType()).contains("image"))
            throw new FileTypeNotImageFileException();
    }
